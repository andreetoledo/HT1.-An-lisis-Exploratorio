...
```{r peliculaMayoresGanancias, echo=FALSE}

```

## Películas de cierto género principal que necesitaron mayor presupuesto

```{r peliMayorPresupuesto, echo=FALSE}
...
## 4.8) Las películas de qué genero principal obtuvieron mayores ganancias

```{r bestGenresRevenue, echo=FALSE}
...
## 4.9) Las películas de qué genero principal obtuvieron mayor presupuesto

```{r bestGenreBudget, echo=FALSE}
...
library(lubridate)
library(ggplot2)
monthsss <- month(as.POSIXlt(sum_movies$release_date, format = "%m/%d/%y"))
barplot(monthsss, main="Meses", xlab="meses")

```

## 4.14)Correlacíon de las calificaciones con el éxito comercial
...
ggplot(data = sum_movies, aes(x = votos), aes(x = ganancias)) + 
  geom_bar() + 
  xlab("Número de votos") + 
  ylab("Ganancias") + 
  ggtitle("éxito comercial")


```

## 4.15) Género de las películas con mayor duración
```{r}
library(dplyr)
library(magrittr)
